...
dEoo = 2.38;
dHOMO = -0.231;
%setting up constants for Rehm-Weller equation; can be modified to use
...
kpet = sqrt(pi/(hbar*lambda*kb*T)).*(HDA^2).*exp(-((lambda+dGpet).^2)./(4*lambda*kb*T));
...
ref = find(Vmem==-60);
dFoverF = (kprot+kpet(ref))./(kprot+kpet)-1;
...
figure(2);
hold on;
plot(Vmem(2001:4001),dFoverF(2001:4001));
plot(Vmem(2001:4001),dFlinfit(2001:4001));

figure(3);
hold on;
plot(Vmem(2001:4001),log10(kpet(2001:4001)));
